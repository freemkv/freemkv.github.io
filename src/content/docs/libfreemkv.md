...
- `Disc` — a scanned disc: titles, streams, format, AACS/CSS state.
- `Disc::scan(...)` — scan a live drive; `Disc::scan_image(...)` — scan an ISO.
- `Disc::sweep(...)` — Pass 1 forward read; `Disc::copy(...)` — sweep or patch, dispatched
  by mapfile state; `Disc::patch(...)` — Pass N recovery.
...
- **CSS (DVD)** is built in — no external key file needed. On a CSS-enforcing drive,
