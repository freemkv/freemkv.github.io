...
| <a class="ecode" id="E7026" href="#E7026">E7026</a> | Error | The forensic (AACS 2.1 / FMTS) index keys for this disc could not be resolved, so it can't be decrypted. | **Cause:** An AACS 2.1 forensic-variant (FMTS) disc needs per-index forensic keys in addition to the base Unit Key. No key source supplied the complete forensic set. **Next steps:** 1. Configure an **online key service** — forensic index keys are online-only (a local `keydb.cfg` does not carry them). 2. Retry once the key service is reachable. 3. In autorip, enable *Capture Discs Without Keys* to save a raw ISO now and decode once keys arrive. |
